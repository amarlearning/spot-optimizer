...
def generate_test_combinations(params: Dict[str, List[Any]], limit: int = None) -> List[Dict[str, Any]]:
    """
    ...
    filtered_cases_extended = [
        case for case in test_cases_extended
        if is_valid_combination(case)
    ]
    
    test_cases_default = [
        dict(zip(default_keys, combo))
        for combo in default_combinations
    ]
    
    filtered_cases_default = [
        case for case in test_cases_default
        if is_valid_combination(case)
    ]
...
def save_test_data(combinations: List[Dict[str, Any]], filename: str = "test_combinations.json"):
    """
