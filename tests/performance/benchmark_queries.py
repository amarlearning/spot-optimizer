...
def analyze_failure_patterns(results: List[Dict[str, Any]]):
    ...
    for core_range in core_ranges:
        for mem_range in memory_ranges:
            ...
            if matching_failures:
                ...
                print(
                    f"Cores {core_range}, Memory {mem_range}: {len(matching_failures)} failures ({percentage:.1f}%)"
                )
    ...
    for region, count in sorted(
        region_failures.items(), key=lambda x: x[1], reverse=True
    ):
        percentage = (count / total_failures) * 100
    ...
    non_arm_failures = len(
        [q for q in failed_queries if not q["params"]["arm_instances"]]
    )

    print(
        f"SSD-only requirement: {ssd_failures} failures ({(ssd_failures/total_failures)*100:.1f}%)"
    )
    print(
        f"Non-ARM requirement: {non_arm_failures} failures ({(non_arm_failures/total_failures)*100:.1f}%)"
    )
    ...
    for r_range in ratio_ranges:
        ...
        if matching_ratios:
            ...
            print(
                f"Memory/Core ratio {r_range}: {len(matching_ratios)} failures ({percentage:.1f}%)"
            )
...
class PerformanceTester:
    ...
    def run_benchmarks(self):
        ...
        if not self.optimizer:
            raise RuntimeError(
                "Environment not prepared. Call prepare_environment() first."
            )
        ...
        for idx, params in enumerate(self.combinations, 1):
            try:
                ...
                results.append({"params": params, "time": query_time, "success": True})

            except Exception as e:
                results.append(
                    {"params": params, "time": None, "success": False, "error": str(e)}
                )
    ...
    def analyze_results(self, results: List[Dict[str, Any]], total_time: timedelta):
        ...
        metrics = {
            ...
            "timing_stats": {
                ...
                "stddev_seconds": (
                    statistics.stdev(query_times) if len(query_times) > 1 else None
                ),
                "p95_seconds": (
