...
@pytest.mark.parametrize(
    "cores, memory, expected_ranges",
    [
        # Small workload - ranges might overlap, that's okay
        (4, 16, {"latency": (1, 1), "balanced": (2, 2), "fault_tolerance": (3, 4)}),
        (8, 32, {"latency": (1, 1), "balanced": (2, 2), "fault_tolerance": (3, 4)}),
        # Medium workload
        (64, 256, {"latency": (1, 1), "balanced": (2, 2), "fault_tolerance": (3, 4)}),
        (128, 512, {"latency": (1, 4), "balanced": (5, 8), "fault_tolerance": (9, 16)}),
        # Large workload
        (
            400,
            2000,
            {"latency": (1, 7), "balanced": (8, 31), "fault_tolerance": (32, 62)},
        ),
        (
            1000,
            4000,
            {"latency": (1, 15), "balanced": (16, 62), "fault_tolerance": (63, 124)},
        ),
        (
            2000,
            10000,
            {"latency": (1, 39), "balanced": (40, 156), "fault_tolerance": (157, 312)},
        ),
    ],
)
def test_mode_ranges(cores, memory, expected_ranges):
    ...
    for mode, (min_instances, max_instances) in ranges.items():
        ...
        ), f"Invalid range for {mode}: {min_instances} > {max_instances}"
...
def test_large_workload_distinct_ranges():
    ...
    assert (
        balanced_min > latency_max
    ), "Balanced range should start after latency range ends"
    assert (
        fault_min > balanced_max
    ), "Fault tolerance range should start after balanced range ends"
